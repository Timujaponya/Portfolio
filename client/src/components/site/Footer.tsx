    ...
    <footer className="bg-slate-950 py-12 text-white">
      <div className="mx-auto w-full max-w-7xl px-4 sm:px-6 lg:px-8">
        <div className="mb-8 grid grid-cols-1 gap-8 md:grid-cols-3">
          ...
          <div>
            ...
            <ul className="space-y-2 text-slate-400">
              <li><a href="#home" className="transition hover:text-white">{t('nav.home')}</a></li>
              <li><a href="#about" className="transition hover:text-white">{t('nav.about')}</a></li>
              <li><a href="#skills" className="transition hover:text-white">{t('nav.skills')}</a></li>
              <li><a href="#projects" className="transition hover:text-white">{t('nav.projects')}</a></li>
              <li><a href="#contact" className="transition hover:text-white">{t('nav.contact')}</a></li>
            </ul>
          </div>

          <div>
            <h4 className="mb-4 font-bold">{t('footer.connect')}</h4>
        ...
        <div className="border-t border-slate-800 pt-8 text-center text-sm text-slate-400">
          <p>
            © {new Date().getFullYear()} <DynamicText text={profile?.name} fallback={t('hero.defaultName')} />. {t('footer.copyright')}
          </p>
        </div>
      </div>
    </footer>
